...
static bool get_nal_payload(const uint8_t* nal, size_t nal_len,
                            const uint8_t** payload, size_t* payload_len) {
    if (!nal || !payload || !payload_len) return false;

    if (nal_len >= 4 && nal[0] == 0 && nal[1] == 0 && nal[2] == 0 && nal[3] == 1) {
        *payload = nal + 4;
        *payload_len = nal_len - 4;
        return *payload_len > 0;
    }

    if (nal_len >= 3 && nal[0] == 0 && nal[1] == 0 && nal[2] == 1) {
        *payload = nal + 3;
        *payload_len = nal_len - 3;
        return *payload_len > 0;
    }
...
static bool process_one_nal(AppState* app, const uint8_t* nal, size_t nal_len) {
    if (!mvd_initialized || !nal || nal_len == 0) return false;

    const uint8_t* payload = NULL;
    size_t payload_len = 0;
    if (!get_nal_payload(nal, nal_len, &payload, &payload_len)) {
        locked_printf(app, "[VIDEO] Invalid Annex-B NAL.\n");
        return false;
    }

    if (payload_len > MAX_NAL_SIZE) {
        locked_printf(app, "[VIDEO] NAL payload too large: %lu\n", (unsigned long)payload_len);
        return false;
    }

    memcpy(mvd_input_buf, payload, payload_len);
    GSPGPU_FlushDataCache(mvd_input_buf, payload_len);

    MVDSTD_ProcessNALUnitOut nal_out;
    memset(&nal_out, 0, sizeof(nal_out));

    Result res = mvdstdProcessVideoFrame(mvd_input_buf, payload_len, 0, &nal_out);

    if (!MVD_CHECKNALUPROC_SUCCESS(res)) {
        locked_printf(app, "[VIDEO] mvdstdProcessVideoFrame failed: 0x%08lX rem=0x%08lX\n",
                      (unsigned long)res, (unsigned long)nal_out.remaining_size);
        return false;
    }

    if (res == MVD_STATUS_PARAMSET || res == MVD_STATUS_INCOMPLETEPROCESSING) {
        return true;
    }
    ...
    mvd_config.physaddr_outdata0 = osConvertVirtToPhys(gfxtopadr);
...
static void video_thread_main(void* arg) {
    ...
    while(app->running) {
        ...
        if (!process_one_nal(app, nalBuf, nal_len)) {
